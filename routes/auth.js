...
router.post("/signup", isLoggedOut, (req, res) => {
  ...
  if (!fullName) {
    return res
      .status(400)
      .render("auth/signup", { errorMessage: "Please provide your fullName." });
  }
  ...
  */
  ...
  // Search the database for a user with the fullName submitted in the form
  User.findOne({ email }).then((found) => {
    // If the user is found, send the message fullName is taken
    if (found) {
      return res
        .status(400)
